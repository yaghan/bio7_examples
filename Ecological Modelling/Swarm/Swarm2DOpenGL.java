...
public Uniform uni=new Uniform(-100.0,100.0,twist);
public int random;
public int width=500;
public int height=500;
public ArrayList boids; // Initialize the arraylist; // An arraylist for all the boids
...
public void ecomain(GL gl, GLU glu, GLUT glut) {
	...
	for (int i = 0; i < boids.size(); i++) {
		...
		b.run(boids, gl, glu, glut); // Passing the entire list of boids to each boid individually
	}
...
class Boid {
	...
	public Boid(PVector l, float ms, float mf) {
		float s1 = (float) uni.nextDouble() / 100;
		float s2 = (float) uni.nextDouble() / 100;
		acc = new PVector(0, 0);
	...
	void run(ArrayList boids, GL gl, GLU glu, GLUT glut) {
	...
	void render(GL gl, GLU glu, GLUT glut) {
...
private void drawTiles(int drawType, GL gl)
...
private void drawTile(int x, int z, GL gl)
