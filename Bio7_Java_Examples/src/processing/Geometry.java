...
public class Geometry extends com.eco.bio7.compile.Model {
	...
	class Embedded extends PApplet {
		...
		public void setup() {
			...
			frameRate(30);
			background(0);
			int[] ruleset = { 0, 1, 0, 1, 1, 0, 1, 0 }; // An initial rule
														// system
		...
		public void draw() {
			...
			if (ca.finished()) { // If we're done, clear the screen, pick a new
									// ruleset and restart
				background(0);
				ca.randomize();
				ca.restart();
	...
	class CA {
		...
		int[] rules; // An array to store the ruleset, for example
						// {0,1,1,0,1,1,0,1}
		...
		void restart() {
			...
			cells[cells.length / 2] = 1; // We arbitrarily start with just the
		...
		void generate() {
			...
			// For every spot, determine new state by examing current state, and
			// neighbor states
		...
		// This is the easy part, just draw the cells, fill 255 for '1', fill 0
		// for '0'
		void render() {
			for (int i = 0; i < cells.length; i++) {
				if (cells[i] == 1)
