...
Info: Added embed.width instead of width (instance variable of PApplet needed). Added 2.0f instead of 2.0 for all floats (Java requirement)!
...
class Embedded extends PApplet {
	...
	public void setup() {
		...
		for (int i = 0; i < 150; i++) {
			flock.addBoid(new Boid(new PVector(
				embed.width / 2,
...
class Boid {
  ...
  // Wraparound
  void borders() {
    if (loc.x < -r) loc.x = embed.width+r;
    if (loc.y < -r) loc.y = embed.height+r;
    if (loc.x > embed.width+r) loc.x = -r;
    if (loc.y > embed.height+r) loc.y = -r;
  }
