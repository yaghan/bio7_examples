...
public void ecomain(GL2 gl, GLU glu, GLUT glut) {
	...
	if (clicked) {
		if (SpatialUtil.isSplitView()) {
			/*
			The following method avoids actions if the split-screen is active!
			The overlay panel is not drawn in the small split-screen panel.
			If true the overlay is drawn in the small panel! 
			 */
